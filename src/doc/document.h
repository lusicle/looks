// Project document. Every mutation goes through doc::Command so undo never
...
// Layer source: the clip or a generator.
enum class LayerSourceKind : uint32_t {
    ...
    // LEGACY, load-compat only (docs/flow_canvas.md): the flat graph
...
struct Layer {
    ...
    // Trim: the segment of the source clip this layer plays,
...
struct Document {
    ...
    // Modulation: global mod matrix + per-param keyframe lanes +
    ...
    // Time remap (speed ramp / reverse / ping-pong). Speed is a
    ...
    // Timeline region: clip trim [in, out) — out 0 = clip end
    ...
    uint32_t loop_in = 0;
    uint32_t loop_out = 0;
    // Timeline markers: plain frame bookmarks, drawn on the ruler,
    // toggled with M at the playhead. Kept sorted.
    ...
    // TRUE GRAPH (docs/flow_canvas.md): first-class links between node
    // outputs and named input ports. Node ids: effect / layer ids; 0 as
    // `to` = the Output node. to_port 0 = In, 1 = the matte port (the
    // wired image gates the consumer through luma extract), 2+ =
    // per-effect aux inputs (Warp/Displace/B...). While `links` is empty
    // the loader and engine synthesize links from the per-layer stack
    // order.
    struct NodeLink {
        uint64_t from = 0;
        uint64_t to = 0;
    ...
    // Canvas frames (docs/flow_canvas.md): titled visual grouping
