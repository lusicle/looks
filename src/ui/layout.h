// Two-pass constraint layout (first-party mirror of the reference
// toolkit's flex layout). The tree is rebuilt into a LayoutArena every
// frame; nodes are trivially destructible and carry C function pointers for
// leaf measure / draw / hit-test.
//
// Pass 1: measure(node, constraints) -> desired size.
// Pass 2: arrange(node, rect) -> final rects + inherited clip.
// Then:   hit pass (register rects with Context, finalize winner),
//         wheel routing (innermost ScrollArea under cursor),
//         draw pass (emit into Canvas2D, honoring node clips),
//         Context::gc_widget_slots().
// run_frame() performs the whole sequence.
...
// Everything a widget callback needs, threaded through the walks.
struct LayoutFrame {
    Canvas2D& canvas;
    UiInput& input;
    Context& ctx;
    ...
    const Theme& theme;
    float dt = 0.0f;
    // Optional display face for headers (serif); null = use `font`.
...
struct LayoutNode {
    ...
    Edges padding;                  // interior inset applied to children
...
// Full frame: layout + hit + wheel + draw + gc.
